...
import { removeUser, clearData } from "../ReduxStore/teamStore";
import axios from "axios";
import { toast } from "react-hot-toast";

const Profile = () => {
  const path = window.location.pathname;
  const teamMembers = useSelector((state) => state.teamState.users);
  ...
  const createTeam = async () => {
    ...
    //   `${process.env.REACT_APP_SERVER_UR}/api/v1/team/`,
    //   {
    //     memberIds: teamMembers.map((member) => member.id),
    //   }
    // );
  
    const response= await fetch(`${process.env.REACT_APP_SERVER_URL}api/v1/team/`,{
      method: 'POST',
      headers: {
  ...
  return (
    <div
      className="
    w-full pb-6 mb-6
    md:flex flex-col bg-white border border-l-black
    md:w-full  text-blue-500 h-full px-4 py-4">
      <h1
  ...
  "
                onClick={() => dispatch(removeUser(member.id))}
              >
                Remove
              </button>
            </div>
